...
pub const fn encrypt128(key: [u8; 16], data: [u8; 8]) -> [u8; 8] {
    ...
    state = round(state, round_keys[0], 2);
    state = round(state, round_keys[1], 3);
    state = round(state, round_keys[0], 4);
    state = round(state, round_keys[1], 5);
    state = round(state, round_keys[0], 6);
    state = round(state, round_keys[1], 7);
    state = round(state, round_keys[0], 8);
    state = round(state, round_keys[1], 9);
    state = round(state, round_keys[0], 10);
    state = round(state, round_keys[1], 11);
...
mod test {
    ...
    fn test() {
        ...
        const ENCRYPTED_DATA: [u8; 8] = encrypt128(KEY, DATA);
